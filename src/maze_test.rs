...
fn test_maze_from_xyz_seed() {
    ...
    for seed in [
        1234, 5678, 9012, 0, 1, 2, 3, 407, 1609, 123456789, 987654321, 192837465, 918273645,
    ] {
        for i in -4..4 {
            let x = i;
            let y = i;
            ...
            let maze = maze_from_xyz_seed(x, y, z, seed, height, width);

            // Ensure the right wall of the current maze matches the left wall of the neighboring maze to the right.
            let maze_x_plus_1 = maze_from_xyz_seed(x + 1, y, z, seed, height, width);
            for h in 0..height {
            ...
            let maze_x_minus_1 = maze_from_xyz_seed(x - 1, y, z, seed, height, width);
            for h in 0..height {
            ...
            let maze_y_plus_1 = maze_from_xyz_seed(x, y + 1, z, seed, height, width);
            for h in 0..height {
                for w in 0..width {
            ...
            let maze_y_minus_1 = maze_from_xyz_seed(x, y - 1, z, seed, height, width);
            for h in 0..height {
                for w in 0..width {
            ...
            let maze_z_plus_1 = maze_from_xyz_seed(x, y, z + 1, seed, height, width);
            for w in 0..width {
            ...
            let maze_z_minus_1 = maze_from_xyz_seed(x, y, z - 1, seed, height, width);
            for w in 0..width {
